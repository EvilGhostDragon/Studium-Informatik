...
uniqueWebsites [] = []
uniqueWebsites (x : xs)  | x `elem` xs = uniqueWebsites xs
                         | otherwise = x : uniqueWebsites xs
{- 9.1.2 Write a higher-order function that counts the 
  number of times a website occurs in a dataset like 
  webData -}
count ::  String -> [String] -> Int
count s ws = length $ (filter (\x -> x == s ) ws)
...
--result [] = []
result xs = sortBy rank(map (\x -> (count x xs, x)) (uniqueWebsites xs))  where
    rank a b 
        |compare a b == LT = GT
        |compare a b == GT = LT
...
uniqueWebsites2 ws = uniqueWebsites (map fst ws)
...
count2 s ws = sum (map snd (filter (\x -> fst x == s) ws))
...
result2 :: (Ord a , Ord b, Num a) => (String -> b -> a) -> (b -> [String]) -> b -> [(a,String)]
result2 c uw xs = sortBy rank(map (\x -> (c x xs, x)) (uw xs))  where
    rank a b 
        |compare a b == LT = GT
        |compare a b == GT = LT
...
all_fold p l = foldr(&&)True(map p l)
...
dig2int_fold = foldr (\x y -> x + 10 * y ) 0 
...
evenProdSum :: [Int] -> Int
evenProdSum xs = sum [(ind + 1 ) * entry | (ind, entry)<- number xs, odd ind]
